...
export class EtlService {
    ...
    async updateAvailableDailySwimming(): Promise<void> {
        const LIST_1 = [];

        this.logger.log('Start: updateAvailableDailySwimming');

        await this.seoulPoolInfoModel.updateMany(
            { pool_code: { $in: LIST_1 } },
            { $set: { available_daily_swimming: 'N' } }
        );

        await this.seoulPoolInfoModel.updateMany(
            { pool_code: { $nin: LIST_1 } },
            { $set: { available_daily_swimming: 'Y' } }
        );

        this.logger.log('Done: updateAvailableDailySwimming');
    }

    async deleteDailySwimScheduleByPoolCodes(): Promise<void> {
        const LIST_1 = [];

        this.logger.log('Start: deleteDailySwimScheduleByPoolCodes');

        const result = await this.dailySwimScheduleModel.deleteMany({
            pool_code: { $in: LIST_1 }
        });
        this.logger.log(`Deleted count: ${result.deletedCount}`);

        this.logger.log('Done: deleteDailySwimScheduleByPoolCodes');
    }

    async updateAvailableHtmlCrawling(): Promise<void> {
        const LIST_2 = [];

        this.logger.log('Start: updateAvailableHtmlCrawling');

        await this.seoulPoolInfoModel.updateMany(
            { pool_code: { $in: LIST_2 } },
            { $set: { available_html_crawling: 'Y' } }
        );

        await this.seoulPoolInfoModel.updateMany(
            { pool_code: { $nin: LIST_2 } },
            { $set: { available_html_crawling: 'N' } }
        );

        this.logger.log('Done: updateAvailableHtmlCrawling');
    }
}
